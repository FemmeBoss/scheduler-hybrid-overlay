...
async function renderPages(pages, containerId, platform) {
  ...
  if (window.injectSetDefaultTimeButtons) {
    console.log('[DEBUG] Calling injectSetDefaultTimeButtons after page render');
    window.injectSetDefaultTimeButtons();
  } else {
    console.warn('[WARNING] injectSetDefaultTimeButtons not found');
  }
}
...
window.saveDefaultTimes = saveDefaultTimes;