...
import { parseCsv } from './features/csvParser.js';  // Import parseCsv function
...
function renderScheduledPosts() {
  ...
  container.innerHTML = futureScheduledPosts.map(post => `
    <div class="scheduled-post" data-post-id="${post.id}">
      ...
      ` : ''}
      <img src="${post.imageUrl || ASSETS.imagePlaceholder}" 
           alt="Scheduled post" 
           class="post-image"
           onerror="this.src='${ASSETS.imagePlaceholder}'">
...
const ASSETS = {
  defaultProfile: window.location.origin + '/assets/images/default-profile.svg',
  imagePlaceholder: window.location.origin + '/assets/images/image-placeholder.svg'
};

// Update previewPosts function
window.previewPosts = async function() {
  ...
  try {
    ...
    // Create preview for each post (not for each page)
    for (const post of posts) {
      console.log(`🎨 Creating preview for post:`, post);
      ...
      const cardHtml = `
        <div class="post-header">
          <div class="post-header-info">
            <h3 class="page-name">Post Preview</h3>
        ...
        <div class="post-image-container">
          <img src="${post.imageUrl}" 
               alt="Post preview" 
               class="preview-image"
               onerror="this.src='${ASSETS.imagePlaceholder}'; this.classList.add('placeholder');" />
        </div>
      ...
      console.log('✅ Preview card added to container');
    }
    ...
    container.innerHTML = '<p class="error">Failed to load preview posts</p>';
    showNotification('Failed to generate previews', 'error');
  }
};

// Helper function to check for watermark
async function checkWatermark(pageId) {
  try {
    const watermarkRef = ref(storage, `watermarks/${pageId}.jpg`);
